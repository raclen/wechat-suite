...
| 平台 | 当前可用性 | 说明 |
|---|---|---|
| Linux / Ubuntu | 支持，当前验证最多 | `./run.sh`、`./run_group_daily.sh` 和自动导出流程按 Linux 环境验证 |
| macOS | 基本可用 | 总结脚本和 Web UI 是 Python/浏览器实现，通常可运行；自动导出依赖 `wechat-decrypt` 的 macOS 密钥提取和解密流程，需要重签名微信、root 权限等额外步骤 |
| Windows | 支持 | 使用仓库根目录 `run.bat` 启动 Web UI；命令行流水线可用 `run_group_daily.bat`。自动导出会查找 `wechat-decrypt\.venv\Scripts\python.exe` |
...
```bat
run_group_daily.bat config.local.yaml
```
...
- `run_group_daily.bat`：Windows 命令行入口脚本
